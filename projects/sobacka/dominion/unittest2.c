...
int main(){
  ...
  // Check that only 17 cards have any supply ( All unsused cards should not have any available supply)
	...
	assertTrue(17-cardCount, __LINE__, failFlag);
  ...
  int e, c, j; // j used for for-loop 
	for (i = 0; i < numPlayers; i++){
		...
		assertTrue(G.deckCount[i] == 10, __LINE__, failFlag);
		if(G.deckCount[i] == 10){
	...
	assertTrue(G.handCount[0] == 0 && G.handCount[1] == 0, __LINE__,  failFlag);
	for (i = 0; i < numPlayers; i++){
		assertTrue(G.handCount[i] == 0, __LINE__,  failFlag); 
		assertTrue(G.discardCount[i] == 0, __LINE__,  failFlag); 
	}
